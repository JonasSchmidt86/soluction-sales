class FixEstoqueVendaTz < ActiveRecord::Migration[7.1]
  def up
    execute <<-SQL
      CREATE OR REPLACE FUNCTION public.tgrf_estoquevenda() RETURNS trigger LANGUAGE 'plpgsql' COST 100 VOLATILE NOT LEAKPROOF AS $BODY$
          ...
          IF TG_OP = 'DELETE' THEN
              ...
              IF COALESCE(OLD.cancelado, FALSE) = TRUE THEN
                  INSERT INTO estoque_logs (cod_empresa, cod_produto, cod_cor, operacao, origem,
                      ...
                      cod_referencia, cod_item, usuario, cod_funcionario, origem_sistema, observacao)
                  VALUES (
                      OLD.COD_EMPRESA, OLD.COD_PRODUTO, OLD.COD_COR, 'DELETE', 'VENDA',
                      0, 0, 0,
                      OLD.COD_VENDA, OLD.COD_ITEM, current_user, V_COD_FUNCIONARIO, V_ORIGEM_SISTEMA,
          ...
          ELSE
              ...
              IF COALESCE(EXISTE,0) = 0 THEN
                  ...
                  VALUES (
                      ...
                      0, NEW.VALORUNITARIO, (timezone('America/Sao_Paulo', now())::date));
              END IF;
              ...
              IF TG_OP = 'INSERT' THEN
                  ...
                  IF TIPOVENDA = 'V' THEN
                      ...
                      VALUES (
                          ...
                          NEW.COD_VENDA, NEW.COD_ITEM, current_user, V_COD_FUNCIONARIO, V_ORIGEM_SISTEMA,
                  ...
                  ELSIF TIPOVENDA = 'T' THEN
                      ...
                      IF COALESCE(EXISTE,0) = 0 THEN
                          ...
                          VALUES (
                              ...
                              NEW.VALORUNITARIO, VL_VENDA, (timezone('America/Sao_Paulo', now())::date));
                      END IF;
      ...
      BEGIN
          ...
          ELSE
              ...
              IF COALESCE(EXISTE,0) = 0 THEN
                  ...
                  VALUES (
                      NEW.COD_COR, NEW.COD_EMPRESA, NEW.COD_PRODUTO, 0, 0, 0,
                      0, NEW.VALORUNITARIO, CURRENT_DATE);
              END IF;
              ...
              IF TG_OP = 'INSERT' THEN
                  ...
                  ELSIF TIPOVENDA = 'T' THEN
                      ...
                      IF COALESCE(EXISTE,0) = 0 THEN
                          ...
                          VALUES (
                              NEW.COD_COR, COD_EMP_TRANS, NEW.COD_PRODUTO, NEW.VALORUNITARIO, 0, 0,
                              NEW.VALORUNITARIO, VL_VENDA, CURRENT_DATE);
                      END IF;
