class UpdateFunctionsWithCodFuncionario < ActiveRecord::Migration[6.1]
  def up
    # Atualiza function de COMPRA para gravar cod_funcionario no log
    execute <<-SQL
      ...
      BEGIN
          ...
          -- Busca cod_funcionario da compra
          IF TG_OP = 'DELETE' THEN
              SELECT C.COD_FUNCIONARIO INTO V_COD_FUNCIONARIO
                FROM COMPRA C WHERE C.COD_COMPRA = OLD.COD_COMPRA;
          ELSE
              SELECT C.COD_FUNCIONARIO INTO V_COD_FUNCIONARIO
                FROM COMPRA C WHERE C.COD_COMPRA = NEW.COD_COMPRA;
          END IF;
    ...
    # Atualiza function de VENDA para gravar cod_funcionario no log
    execute <<-SQL
      ...
      BEGIN
          ...
          -- Busca cod_funcionario da venda
          IF TG_OP = 'DELETE' THEN
  ...
  def down
    # Rollback volta para versão anterior (migrations 000005 e 000006)
  end
end
