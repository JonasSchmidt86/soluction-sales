class FixEstoqueCompraTz < ActiveRecord::Migration[7.1]
  def up
    execute <<-SQL
      CREATE OR REPLACE FUNCTION public.tgrf_estoquecompra() RETURNS trigger LANGUAGE 'plpgsql' COST 100 VOLATILE NOT LEAKPROOF AS $BODY$
      ...
      BEGIN
          ...
          BEGIN
              ...
              V_ORIGEM_SISTEMA = NULLIF(current_setting('app.origem_sistema', true), '');
          ...
          IF TG_OP = 'DELETE' THEN
              ...
              IF OLD.cancelado = TRUE THEN
                  INSERT INTO estoque_logs (cod_empresa, cod_produto, cod_cor, operacao, origem,
                      ...
                      cod_referencia, cod_item, custofinal, usuario, cod_funcionario, origem_sistema, observacao)
                  VALUES (
                      OLD.COD_EMPRESA, OLD.COD_PRODUTO, OLD.COD_COR, 'DELETE', 'COMPRA',
                      0, 0, 0,
                      OLD.COD_COMPRA, OLD.COD_ITEM, NULL, current_user, V_COD_FUNCIONARIO, V_ORIGEM_SISTEMA,
              ...
              ELSE
                  ...
                  VALUES (
                      ...
                      (0 - OLD.QUANTIDADE), 0, OLD.VALORUNITARIO, 0, (timezone('America/Sao_Paulo', now())::date)
                  );
          ...
          ELSE
              ...
              IF EXISTE = 0 THEN
                  ...
                  VALUES (
                      ...
                      COALESCE(CUSTOFINAL,0), COALESCE(NOVO_VALOR_VENDA,0), (timezone('America/Sao_Paulo', now())::date)
                  );
              END IF;
              ...
              UPDATE EMPRESAPRODUTO AS E
                 SET QUANTIDADE = COALESCE(E.QUANTIDADE,0) + ITEMQUANTIDADE,
                     ...
                     QTDFISCAL = CASE
                         ...
                         ELSE E.QTDFISCAL
                     END,
                     DATAALTERACAO = (timezone('America/Sao_Paulo', now())::date)
               WHERE E.COD_PRODUTO = NEW.COD_PRODUTO
                 AND E.COD_COR = NEW.COD_COR
                 AND E.COD_EMPRESA = NEW.COD_EMPRESA;
              ...
              VALUES (
                  ...
                  NEW.COD_COMPRA, NEW.COD_ITEM, CUSTOFINAL, current_user, V_COD_FUNCIONARIO, V_ORIGEM_SISTEMA,
                  CASE TG_OP
      ...
      BEGIN
          ...
          IF TG_OP = 'DELETE' THEN
              ...
              ELSE
                  ...
                  VALUES (
                      OLD.COD_COR, OLD.COD_EMPRESA, OLD.COD_PRODUTO, (0 - OLD.VALORUNITARIO),
                      (0 - OLD.QUANTIDADE), 0, OLD.VALORUNITARIO, 0, CURRENT_DATE
                  );
          ...
          ELSE
              ...
              IF EXISTE = 0 THEN
                  ...
                  VALUES (
                      NEW.COD_COR, NEW.COD_EMPRESA, NEW.COD_PRODUTO, COALESCE(CUSTOFINAL,0), 0, 0,
                      COALESCE(CUSTOFINAL,0), COALESCE(NOVO_VALOR_VENDA,0), CURRENT_DATE
                  );
              END IF;
