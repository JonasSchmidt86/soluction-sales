class CollaboratorsBackoffice::Report::RepAtendimentosController < CollaboratorsBackofficeController
  ...
  def index
    ...
    query = query.where(
      "DATE(attended_at) BETWEEN TO_DATE(?, 'DD/MM/YYYY') AND TO_DATE(?, 'DD/MM/YYYY')",
      data_inicial, data_final
    )

    if params[:term].present?
      query = query.where("atendimentos.name ILIKE ?", "%#{params[:term]}%")
    end

    if params[:cod_funcionario].present?
      query = query.where(employee_id: params[:cod_funcionario])
    end
    ...
    @atendimentos = if params[:per_page] == 'Todas'
      query.order(attended_at: :desc)
    else
      query.order(attended_at: :desc).page(params[:page]).per(per_page)
    end

    @total   = @atendimentos.respond_to?(:total_count) ? @atendimentos.total_count : @atendimentos.size
    @venderam = query.where(sold: true).count
  end
